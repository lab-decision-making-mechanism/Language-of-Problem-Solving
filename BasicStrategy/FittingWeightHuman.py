...
from Utils.FileUtils_fmri import readAdjacentMap, readLocDistance
...
def _combine(cutoff_pts, dir, is_nan):
    '''
        ...
        for i in range(1, len(cutoff_pts)):
            if cutoff_pts[i][1] - cutoff_pts[i][0] > 3:
                ...
                if is_nan[i] == False and pre_nan == False:
                    ...
                    temp_pts.append(cutoff_pts[i])
                    new_is_nan.append(is_nan[i])
            pre_nan = new_is_nan[-1]
        cutoff_pts = copy.deepcopy(temp_pts)
        is_nan = copy.deepcopy(new_is_nan)
    new_cutoff_pts = []
    new_is_nan = []
    i = 0
    while i < len(cutoff_pts):
        if i == len(cutoff_pts) - 1:
            new_cutoff_pts.append(cutoff_pts[i])
            new_is_nan.append(is_nan[i])
            break
        if cutoff_pts[i][1] - cutoff_pts[i][0] > 3 or is_nan[i] == True:
            new_cutoff_pts.append(cutoff_pts[i])
            new_is_nan.append(is_nan[i])
        elif cutoff_pts[i][1] - cutoff_pts[i][0] <= 3 and is_nan[i] == False:
            if is_nan[i + 1] == False:
                new_cutoff_pts.append((cutoff_pts[i][0], cutoff_pts[i + 1][1]))
                new_is_nan.append(False)
                i += 1
            else:
                new_cutoff_pts.append(cutoff_pts[i])
                new_is_nan.append(is_nan[i])
        i += 1
    return new_cutoff_pts, new_is_nan


def all_directions_nan(data, context):
    pre = context[0]
    end = context[1]
    all_data = data.iloc[pre:end]
...
def combine_context(cutoff_pts, event, data):
    """
    To merge short paragraphs, follow the following merging rules
     1. If the paragraph directions are all empty and the length of the paragraph is less than stay_length, it needs to be merged. If the paragraph event is 1 or 2 and all moving directions of the paragraph are judged to be illegal directions or nan, it needs to be merged. If the paragraph event is 3 and the length is less than 3 Or if all the movement directions of this paragraph are judged to be illegal directions or nan, they need to be merged. In other cases, they will not be merged.
     2. Different types of event paragraphs can be merged into other paragraphs: 0: [1, 2, 3], 1: [1], 2: [0, 2, 3], 3: [0, 1, 2, 3], which means that if the paragraph type is 0, other paragraphs with a paragraph type of 123 can be merged, and so on.
     3. When a paragraph needs to be merged into other paragraphs, first determine whether the preceding and following paragraphs can accept the paragraph. If at least one of them is acceptable, merge it into the shortest paragraph. If neither the preceding nor the preceding paragraph is acceptable, determine whether the current paragraph can become an independent paragraph ( If all the movement directions of this paragraph are judged to be illegal directions or nan, it cannot be formed into an independent paragraph, otherwise it can be formed into an independent paragraph)
         If possible, it will become an independent paragraph, otherwise it will be merged into other paragraphs (if the paragraph is the last paragraph, it will be merged into the previous paragraph, if it is the first paragraph, it will be merged into the next paragraph, if it is a middle paragraph, if the type of the previous paragraph is 1, it will be merged If the type of the next paragraph is 1, it will be merged into the next paragraph).
     4. Incorporate the event of modifying the new paragraph with a priority of 1230
    """
...
def negativeLikelihood(
    ...
    # agent_weight = [3.125e-05, 0, 0, 0, 0, 0, 0, 0]
...
def negativeLikelihoodMergeGroup(
    ...
    # print('line229 nll:', nll)  # for debug
    if not return_trajectory:
        return nll
    else:
        return (nll, dir_Q_value)
...
def caculate_correct_rate(result_x, all_data, true_prob, agents, suffix="_Q"):
    ...
    correct_rate_ = correct_rate_ / 100
...
def fitting_weight_ga_parallelize(idx, cutoff_pts, is_nan, df_monkey, suffix, agents):
    ...
    if all_data.shape[0] == 0:
        ...
        return None
    ind = np.where((temp_data.nan_dir == False) & (temp_data.available_dir == True))[0] + prev
...
def fit_func(df_monkey, cutoff_pts, is_nan, suffix="_Q", is_match=False,
    ...
    results = [fitting_weight_ga_parallelize(idxs[0], cutoff_pts=cutoff_pts, is_nan=is_nan, df_monkey=df_monkey,
    ...
    # for idx in range(len(cutoff_pts)):
    #     fitting_weight_ga_parallelize(idx, cutoff_pts, is_nan, df_monkey, suffix)
...
def merge_fit_func(df_monkey, cutoff_pts, suffix="_Q", is_match=False,
    ...
    for prev, end in cutoff_pts:
        ...
        reverse_group_idx = {each: [None, 0] for each in range(len(agents))}
        ...
        for par in range(len(bounds)):
            ...
            cons.append(l)
            cons.append(u)
        # print('line 307 prev is {}, end is {}:'.format(prev, end))  # for debug
        ...
        # -----
        reassigned_weights = [res.x[reverse_group_idx[i][0]] / reverse_group_idx[i][1] for i in range(len(agents))]
        ...
        phase_is_correct, predict_dir = _calculate_is_correct(reassigned_weights, all_data, true_prob, agents,
                                                              suffix=suffix)
        is_correct[ind] = phase_is_correct
        pre_dir[ind] = predict_dir
    if is_match:
        return result_list, total_loss, is_correct, trial_same_dir_groups, pre_dir
    else:
        return result_list, total_loss
...
def merge_context(context, cutoff_pts):
    """
    Add the paragraphs that are all nan to cutoff_pts, and ensure that the current paragraphs that are all nan are not cut.
    """
    new_cutoff_pts = [0]
    pointer = 0
    end = -1
...
def add_cutoff_pts(cutoff_pts, df_monkey):
    ...
    eat_ghost = (
        (
                ...
                | ((df_monkey.ifscared3 == 3) & (df_monkey.ifscared3.diff() < 0))
                | ((df_monkey.ifscared4 == 3) & (df_monkey.ifscared4.diff() < 0))
        )
    ...
    # Merge the ghost-eaten paragraphs
    eat_ghost_context = []
...
def _readData(filename):
    ...
    filename_list = df.DayTrial.unique()  # change1
    # selected_data = pd.concat([df[df.file == i] for i in filename_list]).reset_index(drop=True)
    selected_data = pd.concat([df[df.DayTrial == i] for i in filename_list]).reset_index(drop=True)  # change2
    df = selected_data
    for cols in ['pacmanPos', 'ghost1Pos', 'ghost2Pos',
                 'ghost3Pos', 'ghost4Pos', 'beans', 'energizers']:
        df[cols] = df[cols].apply(lambda x: eval(x) if isinstance(x, str) else x)
    # -----------------
    # Drop some columns
    ...
    if "global_Q" in df.columns and "global_inf_Q" in df.columns:
        ...
        if is_7str == 1:
            df = df.drop(columns=['global_Q', 'local_Q', 'evade_blinky_Q', 'evade_clyde_Q', 'approach_Q', 'energizer_Q',
                                  'no_energizer_Q',
                                  'global_Q_norm', 'local_Q_norm', 'evade_blinky_Q_norm', 'evade_clyde_Q_norm',
                                  'approach_Q_norm', 'energizer_Q_norm', 'no_energizer_Q_norm'])
            ...
            df = df.drop(
                columns=['global_Q', 'local_Q', 'evade_blinky_Q', 'evade_clyde_Q', 'evade_ghost3_Q', 'evade_ghost4_Q',
                         'approach_Q', 'energizer_Q',
                         'global_Q_norm', 'local_Q_norm', 'evade_blinky_Q_norm', 'evade_clyde_Q_norm',
                         'evade_ghost3_Q_norm', 'evade_ghost4_Q_norm',
                         'approach_Q_norm', 'energizer_Q_norm'])
            df = df.rename(columns={'global_inf_Q': "global_Q", 'local_inf_Q': 'local_Q',
                                    'evade_blinky_inf_Q': 'evade_blinky_Q', 'evade_clyde_inf_Q': 'evade_clyde_Q',
                                    'evade_ghost3_inf_Q': 'evade_ghost3_Q', 'evade_ghost4_inf_Q': 'evade_ghost4_Q',
                                    'approach_inf_Q': 'approach_Q', 'energizer_inf_Q': 'energizer_Q'})
    # -----------------
...
def context_event(x, cutoff_pts, eat_energizer, eat_ghost):
    """
    Event stamp for each paragraph, 0 means all nan, 1 means energizer-eaten, 2 means ghost-eaten, 3 means ordinary paragraph
    """
    event = []
    is_nan = []
    ee_pointer = 0  # energizer pointer
    eg_pointer = 0  # ghost pointer
    # and cutoff_pts[i][1] - cutoff_pts[i][0] > 5
    for i in range(len(cutoff_pts)):
        if np.all(x.iloc[cutoff_pts[i][0]:cutoff_pts[i][1]].apply(lambda x: isinstance(x, float)) == True):
            # If the paragraph directions are all nan
            # Mark event as 0
            is_nan.append(True)
            event.append(0)

            # Move energizer and ghost pointers
            ...
            # Determine whether the paragraph is energizer or ghost and move the energizer and ghost pointers
            if ee_pointer < len(eat_energizer) and eat_energizer[ee_pointer] > cutoff_pts[i][0] and eat_energizer[
                ee_pointer] <= cutoff_pts[i][1]:
                event.append(1)
                while (eat_energizer[ee_pointer] > cutoff_pts[i][0] and eat_energizer[ee_pointer] <= cutoff_pts[i][1]):
...
def dynamicStrategyFitting(config):
    ...
    df["available_dir"] = df[['pacmanPos', 'next_pacman_dir_fill']].apply(
        lambda x: is_available(x.pacmanPos, x.next_pacman_dir_fill), axis=1)
    nan_index = np.where(df["available_dir"] == False)[0]
    df["next_pacman_dir_fill"].iloc[nan_index] = [np.nan] * len(nan_index)
...
def FittingWeight(filename, saveFolder):
    print(filename)
    config = {
        "filename": filename,
...
def get_human_weight(date):
    fileFolder = "../HumanData/CorrectedUtilityData/" + date + "/"
    saveFolder = "../HumanData/WeightData/" + date + "/"
